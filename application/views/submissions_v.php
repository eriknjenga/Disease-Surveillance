...
?>
<script >
	//json object to retrieve the site url
	json_obj = {
	...
	function getDistricts(){
		/*
		 * this method retrieves the districts in particular province and places them in a dropdown list depending on the province the user has selected
		 */
		var province_id=document.getElementById("province").value;
		if(province_id==0){
			//if no province is selected,the district dropdown is initialized to have one value;'Selected District'
	...
	<?php echo form_open('Submissions_c/filter'); ?>
		<table>
			<tr>
				...
				<td> Year 
					<select name="filteryear" id="filteryear"><option selected>--Select Year--
						<?php
