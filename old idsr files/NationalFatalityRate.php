...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".Fatality_Rate_Epiweek").change(function(){
            var years = $(".Fatality_Rate_Year");
            var year = "";
            for(x=0; x<years.size();x++){
                var current = $(years[x]).css("font-weight"); 
    ...
    function getFatalityRate(year){
        var epiweek = $("#Fatality_Rate_Epiweek").attr('value'); 
        var disease = $("#Fatality_Rate_Disease").attr('value'); 
        var province = $("#Fatality_Rate_Province").attr('value');
        var url = "xml/fatality_rate.php?parameters="+disease+"_"+year+"_"+province+"_"+epiweek+" ";
...
<div class="section">
    ...
    <div class="xtop">
        <div id="right_container">
            ...
            <div class="section-title">Case Fatality Rates on a National Basis</div>
            <table>
                <tr>
	...
	Prov: <select name="Fatality_Rate_Province" id="Fatality_Rate_Province" class="Fatality_Rate_Province">
                ...
                Epiweek: <select name="Fatality_Rate_Epiweek" id="Fatality_Rate_Epiweek" class="Fatality_Rate_Epiweek">
                    <?php
                    $sql_epiweeks = "SELECT DISTINCT epiweek FROM surveillance";
                    $sql_result_epiweeks = mysql_query($sql_epiweeks) or die (mysql_error());
                    while($epiweek_resultset = mysql_fetch_assoc($sql_result_epiweeks)){
                        ?>
                        <option value="<?php echo $epiweek_resultset['epiweek'] ?>"><?php echo $epiweek_resultset['epiweek']; ?>
                        </option>
                        <?
                        mysql_data_seek($sql_result_epiweeks);
                    }
                    ?>
                </select>
        ...
        <script type="text/javascript">
            var chart = new FusionCharts("FusionCharts/Charts/Line.swf", "ChartId", "900", "550", "0", "1");
            chart.setDataURL("xml/national_fatality_rate.php");	
            chart.addParam("WMode", "Transparent");	   
            chart.render("Fatality_Rate_Chart");
