...
while ($disease = mysql_fetch_assoc($sql_result)) {
    ...
    if ($disease['description'] == 'Malaria') {
        $diseases[$disease['id']] = $disease['description'];
        $diseases["tested"] = "tested";
        $diseases["positive"] = "positive";
        echo "<th rowspan=2>" . $disease['description'] . "</th>";
...
//echo $sql_surveillance;
                            ...
                            $sql_reports_expected = "select expected from surveillance where district = '" . $district['ID'] . "' AND epiweek='" . $epiweek . "'";
							$sql_reports_submitted = "select submitted from surveillance where district = '" . $district['ID'] . "' AND epiweek='" . $epiweek . "'";
                            ...
                            $available_data['reports'] = $array_expected_reports['expected'];
							 $sql_result_submitted = mysql_query($sql_reports_submitted);
                            $array_submitted_reports = mysql_fetch_assoc($sql_result_submitted);
							 $available_data['submitted'] = $array_submitted_reports['submitted'];
        ...
        <table class="data-table">
            ...
            while ($disease = mysql_fetch_assoc($sql_result)) {
                ...
                if ($disease['description'] == 'Malaria') {
                    ...
                    $diseases["positive"] = "positive";
                    echo "<th rowspan=2>" . $disease['description'] . "</th>";
                    echo "<th  colspan=2 style='color:green;'>" . $disease['description'] . " Indicators</th>";
                } else {
                    $diseases[$disease['id']] = $disease['description'];
                    echo "<th rowspan=2>" . $disease['description'] . "</th>";
                }
